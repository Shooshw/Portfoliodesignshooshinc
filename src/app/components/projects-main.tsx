...
export function ProjectsMain({ onProjectClick }: ProjectsMainProps) {
  ...
  return (
    <section
      id="projects"
      className={`py-32 relative transition-colors duration-700 ${bg}`}
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="max-w-2xl">
            ...
            <h2
              className={`font-display font-semibold italic text-[clamp(2.8rem,5vw,4.5rem)] tracking-tighter leading-[0.9] ${textPrimary}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 max-w-5xl mx-auto">
          {mainProjects.map((project, index) => {
            ...
            const isLol =
              project.title === "League of Legends" || project.id === 2;
              ...
              >
                <div className="relative h-[420px] overflow-hidden bg-[#0D0D0D]">
                  {isAquora ? (
                    <>
                      ...
                      <video
                        preload="metadata"
                        src="/images/List-9-16.mp4"
                        loop
                        playsInline
                        muted
                        className="absolute inset-0 w-full h-full object-cover scale-[1.10] group-hover:scale-[1.13] transition-transform duration-700 z-0"
                      />
                    </>
                  ) : isLol ? (
                    <>
                      ...
                      <video
                        ...
                        playsInline
                        muted
                        className="absolute inset-0 w-full h-full object-cover scale-[1.5] transition-transform duration-700 z-0"
                      />
                    </>
                  ) : project.image ? (
                    <ImageWithFallback
                    ...
                    <div
                      className={`w-full h-full bg-gradient-to-br ${project.color} opacity-40 transition-transform duration-700 group-hover:scale-[1.03]`}
                    />
                  )}
                ...
                <div className="p-8 pb-10 flex flex-col relative bg-inherit z-30">
                  <div className="flex items-center justify-between gap-4 mb-4">
                    <h3
                      className={`font-display font-semibold italic text-2xl tracking-tighter transition-colors group-hover:text-[#C8392B] ${textPrimary}`}
                    >
                      {project.translations?.[language]?.title || project.title}
                    </h3>
                    <span
                      className={`font-sans text-[0.65rem] font-bold tracking-widest opacity-30 ${textPrimary}`}
                    >
                      {project.year}
                    </span>
                  </div>
                  <p
                    className={`font-sans text-sm leading-relaxed mb-8 opacity-60 line-clamp-2 ${textPrimary}`}
                  >
                  ...
                  <div className="relative group/tooltip">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {project.tools.length > 3 && (
                        <span
                          className={`px-3 py-1 rounded-lg font-sans text-[0.6rem] font-bold tracking-wider uppercase border border-transparent ${textMuted}`}
                        >
                          +{project.tools.length - 3}
                        </span>
                      )}
                    ...
                    {project.tools.length > 3 && (
                      <div
                        className={`absolute -top-10 left-0 ${tooltipBg} ${tooltipText} text-[0.6rem] font-bold tracking-wide uppercase px-3 py-1.5 rounded opacity-0 group-hover/tooltip:opacity-100 group-hover/tooltip:-translate-y-1 transition-all duration-300 pointer-events-none z-50 whitespace-nowrap shadow-xl`}
                      >
                        {project.tools.join(" • ")}
                      </div>
                    )}
