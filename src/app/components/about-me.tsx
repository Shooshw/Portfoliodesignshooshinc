    ...
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2
            className={`font-display font-semibold italic text-[clamp(3.5rem,8vw,6rem)] tracking-tighter leading-[0.85] ${textPrimary}`}
          >
            {t("about.title1")}
            <br />
              ...
              >
                ...
                <div className="space-y-12">
                  {[1, 2].map((num) => (
                    <div
                      key={num}
                      className="relative pl-10 border-l border-[#C8392B]/20"
                    >
                      <div className="absolute top-0 left-[-4px] w-2 h-2 rounded-full bg-[#C8392B]" />
                      <p
                        className={`font-sans text-[0.65rem] font-bold tracking-widest uppercase mb-2 ${textMuted}`}
                      >
                        {t(`academic.degree${num}.period`)}
                      </p>
                      <h4
                        className={`font-display font-semibold text-xl mb-1 ${textPrimary}`}
                      >
                ...
                <div className="space-y-12">
                  {[1, 2].map((num) => (
                    ...
                    >
                      ...
                      <p
                        className={`font-sans text-sm opacity-60 leading-relaxed ${textPrimary}`}
                      >
                        {t(`experience.job${num}.description`)}
                        {t(`experience.job${num}.company`) &&
                          ` · ${t(`experience.job${num}.company`)}`}
                      </p>
                    </div>
                  ))}
          ...
          <div className="lg:col-span-12 xl:col-span-5 space-y-12">
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3
                  className={`font-display font-semibold text-2xl ${textPrimary}`}
                >
              ...
              <ul className="space-y-4">
                {[1, 2, 3, 4].map((num) => (
                  <li
                    key={num}
                    className={`font-sans text-sm leading-relaxed pb-4 border-b ${border} last:border-0 ${textPrimary} opacity-60`}
                  >
                    {t(`cert.${num}`)}
                  </li>
                ))}
            ...
            >
              ...
              <div className="space-y-4 text-sm">
                {languages.map((lang, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center py-3 border-b last:border-0 border-current/5"
                  >
                    <span className={`font-sans font-medium ${textPrimary}`}>
                      {lang.name}
                    </span>
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-4">
                {[1, 2, 3, 4].map((num) => (
                  <div
                    key={num}
                    className={`p-4 rounded-2xl border ${border} ${isDark ? "bg-white/2" : "bg-black/2"}`}
                  >
                    <p
                      className={`font-sans text-xs italic ${textPrimary} opacity-60`}
                    >
                      {t(`pref.${num}`)}
                    </p>
                  </div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 justify-center lg:justify-start">
              {technicalSkills.map((skill, index) => (
                <span
                  key={index}
                  className={`px-6 py-3 rounded-2xl border font-sans text-xs font-bold tracking-widest uppercase transition-all hover:bg-[#C8392B] hover:text-white hover:border-[#C8392B] ${textPrimary} ${border}`}
                >
                  {skill}
                </span>
              ))}
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 justify-center lg:justify-start">
              {softSkills.map((skill, index) => (
                <span
                  key={index}
                  className={`px-6 py-3 rounded-2xl border font-sans text-xs font-bold tracking-widest uppercase border-[#C8392B]/20 text-[#C8392B] bg-[#C8392B]/[0.02]`}
                >
                  {skill}
                </span>
              ))}
