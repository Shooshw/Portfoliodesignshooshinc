...
export function Header() {
  ...
  const navLinks = [
    {
      name: t("sidebar.about") || "Sobre Mim",
      href: "#about-intro",
      icon: User,
    },
    {
      name: t("sidebar.projects") || "Projetos",
      href: "#projects",
      icon: FolderOpen,
    },
        ...
        <div className="mx-auto w-full max-w-[1500px] flex items-center justify-between px-6 lg:px-12 py-4">
          ...
          <div className="flex items-center gap-4 sm:gap-6 z-10">
            ...
            >
              <Menu
                size={24}
                className={`${textMuted} group-hover:text-white transition-colors`}
              />
            </motion.button>

            <button
              onClick={handleReturnHome}
              className="flex flex-col select-none group text-left pt-0.5"
            >
              <span
                className={`font-display font-semibold italic text-xl md:text-2xl leading-none tracking-tighter transition-colors group-hover:text-white ${textPrimary}`}
              >
                Shoosh.inc
              </span>
            </button>
          ...
          <div className="flex items-center gap-2 shrink-0 z-10">
            ...
            >
              {isDark ? (
                <Sun size={15} className="opacity-80" />
              ) : (
                <Moon size={15} className="opacity-80" />
              )}
            </motion.button>
          </div>
        </div>
