...
export function CVPage() {
  ...
  const handleDownloadCV = async () => {
    const cvUrl = "/imports/Curriculo_design_PTBR2026-1.pdf";
    try {
  ...
  const skills: SkillWithTooltip[] = [
    ...
    {
      key: "branding",
      name: t("skill.branding"),
      description: t("skill.branding.desc"),
    },
    {
      key: "illustration",
      name: t("skill.illustration"),
      description: t("skill.illustration.desc"),
    },
    {
      key: "prototyping",
      name: t("skill.prototyping"),
      description: t("skill.prototyping.desc"),
    },
    {
      key: "imageEditing",
      name: t("skill.imageEditing"),
      description: t("skill.imageEditing.desc"),
    },
    {
      key: "motion",
      name: t("skill.motion"),
      description: t("skill.motion.desc"),
    },
  ];

  const toolsConfigs = [
    {
      category: t("tools.design"),
      items: ["Figma", "Illustrator", "Photoshop", "InDesign", "Affinity"],
    },
    ...
    {
      category: t("tools.other"),
      items: ["Canva", "VSCode", "Google Workspace"],
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between mb-32 gap-12">
          ...
          >
            ...
            <h1
              className={`font-display font-semibold italic text-[clamp(3.5rem,9vw,6rem)] tracking-tighter leading-[0.85] mb-8 ${textPrimary}`}
            >
              {t("cv.title").split(" ")[0]}
              <br />
              <span className="opacity-30">
                {t("cv.title").split(" ").slice(1).join(" ")}
              </span>
            </h1>
            <p
              className={`font-sans text-xl md:text-2xl font-light tracking-tight ${textSecondary}`}
            >
              {t("cv.subtitle")}
            </p>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-12 xl:col-span-8 space-y-24">
            ...
            <section>
              <div className="flex items-center gap-6 mb-12">
                ...
                <h2
                  className={`font-display font-semibold italic text-3xl ${textPrimary}`}
                >
                  {t("cv.summary")}
                </h2>
              </div>
              <p
                className={`font-sans text-lg md:text-xl leading-relaxed font-light ${textSecondary}`}
              >
                {t("cv.summaryText")}
              </p>
              ...
              <div className="space-y-16">
                {experiences.map((exp, idx) => (
                  ...
                  >
                    ...
                    <span
                      className={`font-sans text-[0.65rem] font-bold tracking-[0.3em] uppercase mb-4 block text-[#C8392B]`}
                    >
                      {exp.period}
                    </span>
                    <h3
                      className={`font-display font-semibold italic text-2xl mb-2 ${textPrimary}`}
                    >
                      {exp.title}
                    </h3>
                    <p
                      className={`font-sans text-sm font-bold opacity-40 mb-6 uppercase tracking-wider ${textPrimary}`}
                    >
                      {exp.company}
                    </p>
                    <p
                      className={`font-sans text-base leading-relaxed font-light ${textSecondary}`}
                    >
                      {exp.description}
                    </p>
                  </motion.div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {education.map((edu, idx) => (
                  ...
                  >
                    <span
                      className={`font-sans text-[0.6rem] font-bold tracking-[0.25em] uppercase mb-4 block ${textMuted}`}
                    >
                      {edu.period}
                    </span>
                    <h3
                      className={`font-display font-semibold text-xl mb-3 ${textPrimary}`}
                    >
                      {edu.title}
                    </h3>
                    <p
                      className={`font-sans text-sm opacity-60 ${textPrimary}`}
                    >
                      {edu.institution}
                    </p>
                  </div>
          ...
          <aside className="lg:col-span-12 xl:col-span-4 space-y-12">
            {/* Essential Skills */}
            <div
              className={`p-10 rounded-[3rem] border transition-colors ${surface} ${border}`}
            >
              <div className="flex items-center gap-4 mb-10">
                <Palette size={20} className="text-[#C8392B]" />
                <h3
                  className={`font-display font-semibold italic text-2xl ${textPrimary}`}
                >
                  {t("cv.skills")}
                </h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <div key={skill.key} className="relative group">
                    ...
                    >
                      <p className="font-bold mb-1 uppercase tracking-widest text-[#C8392B]">
                        {skill.name}
                      </p>
                      {skill.description}
                    </div>
            ...
            >
              ...
              <div className="space-y-10">
                {toolsConfigs.map((cat, idx) => (
                  <div key={idx}>
                    <p
                      className={`font-sans text-[0.6rem] font-bold tracking-[0.3em] uppercase mb-4 ${textMuted}`}
                    >
                      {cat.category}
                    </p>
                    <div className="flex flex-wrap gap-2 text-primary">
                      {cat.items.map((item, i) => (
                        <span
                          key={i}
                          className={`px-4 py-2 border rounded-xl font-sans text-xs font-medium ${border} ${textSecondary}`}
                        >
                          {item}
                        </span>
                      ))}
            ...
            >
              ...
              <div className="space-y-6 mb-12">
                {languages.map((lang, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col gap-1 border-b last:border-0 border-current/5 pb-4"
                  >
                    <span
                      className={`font-sans text-sm font-bold uppercase tracking-widest ${textPrimary}`}
                    >
                      {lang.name}
                    </span>
                    <span
                      className={`font-sans text-[0.65rem] font-bold text-[#C8392B] opacity-70`}
                    >
                      {lang.level}
                    </span>
                  </div>
              ...
              <ul className="space-y-4">
                {certifications.map((cert, idx) => (
                  <li
                    key={idx}
                    className={`font-sans text-xs leading-relaxed opacity-60 flex gap-3 ${textPrimary}`}
                  >
