...
export function Rmp() {
  ...
  const [phaseText, setPhaseText] = useState(
    "Desenvolvida por Edmund Jacobson, a técnica induz o relaxamento mental por meio da percepção e relaxamento muscular.",
  );
  ...
  useEffect(() => {
    if (!isActive) {
      setPhaseText(
        "Desenvolvida por Edmund Jacobson, a técnica induz o relaxamento mental\npor meio da percepção e\nrelaxamento muscular.",
      );
      return;
    }
  ...
  return (
    <div className="bg-gradient-to-b from-[#1c5541] to-[#25463f] relative w-full h-[600px] mx-auto overflow-hidden font-sans max-w-[400px] shadow-2xl flex flex-col items-center rounded-3xl">
      ...
      <div className="absolute bottom-0 left-0 w-full h-[45%] pointer-events-none z-0">
        ...
        >
          <path
            d="M-50,300 L-50,200 Q80,50 200,220 T400,100 L400,300 Z"
            fill="rgba(255,255,255,0.06)"
          />
          <path
            d="M120,300 L120,250 Q250,80 400,200 L400,300 Z"
          ...
          <div className="flex items-center justify-center gap-[60px] mb-8">
            ...
            >
              <Play
                ...
                className={!isActive ? "fill-white/60" : ""}
              />
            </button>
...
export function Grounding() {
  const [isActive, setIsActive] = useState(false);
  const [phaseText, setPhaseText] = useState(
    "Conecte-se com o presente. Ative o som para frequências de base.",
  );
  const [soundEnabled, setSoundEnabled] = useState(true);
  const soundEnabledRef = useRef(soundEnabled);
