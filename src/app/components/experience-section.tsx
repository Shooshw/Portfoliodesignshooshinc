...
export function ExperienceSection() {
  ...
  const experiences = [
    { period: t("experience.job1.period"), title: t("experience.job1.title"), description: t("experience.job1.description"), company: "" },
    { period: t("experience.job2.period"), title: t("experience.job2.title"), description: "", company: t("experience.job2.company") },
  ];

  return (
    <section className={`py-32 relative overflow-hidden transition-colors duration-700 ${bg}`}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid gap-10">
          {experiences.map((exp, index) => (
            ...
            >
              <div 
                className="w-20 h-20 rounded-3xl flex items-center justify-center shrink-0 transition-all duration-500 group-hover:bg-[#C8392B] bg-[#C8392B]/5 border border-[#C8392B]/20"
              >
                <Briefcase size={32} className="text-[#C8392B] group-hover:text-white transition-colors" />
              </div>
              
              <div className="flex-grow">
                <div className="flex flex-wrap items-center gap-4 mb-3">
                  <h3 className={`font-display font-semibold italic text-3xl leading-tight ${textPrimary}`}>
                    {exp.title}
                  </h3>
                  {exp.company && (
                ...
                <div className={`flex items-center gap-2 mb-8 font-sans text-xs font-bold tracking-widest uppercase ${textMuted}`}>
                  <Calendar size={14} className="opacity-40" />
                  <span>{exp.period}</span>
                </div>

                {exp.description && (
                  <p className={`max-w-3xl font-sans text-base leading-relaxed opacity-60 ${textPrimary}`}>
                    {exp.description}
                  </p>
                )}
              </div>
              ...
              <div className="hidden lg:block opacity-0 group-hover:opacity-20 transition-opacity">
                <p className={`font-display italic text-6xl select-none ${textPrimary}`}>0{index + 1}</p>
              </div>
            </motion.div>
          ))}
        </div>
